...
export const AddUserImage = ({ profileImage, className }: Props) => {
    ...
    const onImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            if (result.success) {
                ...
                errors?.forEach((error: any) => form.setError("image", { message: error }));
            }
        }
    };
    ...
    const { startUpload, isUploading } = useUploadThing("imageUploader", {
        ...
        onClientUploadComplete: (data: { url: string; }[]) => {
            ...
            <Dialog open={open} onOpenChange={setOpen}>
                <DialogTrigger asChild>
                    ...
                    >
                        ...
                        <div className="group-hover:opacity-80 transition-opacity duration-200 opacity-0 w-full h-full absolute bg-black flex justify-center items-center flex-col gap-1 text-xs  dark:text-black">
                            <Camera size={20} />
                            <p>{t("HOVER")}</p>
                        </div>
                    </Button>
                ...
                <DialogContent className="flex flex-col items-center justify-center sm:max-w-[28rem] p-0">
                    ...
                    {imagePreview ? (
                        ...
                        <UserAvatar
                            className="w-32 h-32 sm:w-52 sm:h-52 my-5 flex items-center justify-center"
                            size={52}
                    ...
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)}>
                            ...
                            <div className="flex mt-5 w-full justify-center items-center gap-4 pb-4">
                                <Button
                                    type="button"
                                    disabled={!imageOptions.canDelete || isDeleting}
                                    variant={imageOptions.canDelete ? "default" : "destructive"}
                                ...
                                >
                                    {isPending || isUploading ? (
                                        <LoadingState className="flex items-center justify-center" />
                                    ) : (
                                        <Check size={18} />
                                    )}
                                </Button>
